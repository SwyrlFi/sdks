import { Currency, Ether, NativeCurrency, Token } from '@swyrlfi/sdk-core';
...
export const WRAPPED_NATIVE_CURRENCY: { [chainId in ChainId]: Token } = {
  ...
  [ChainId.MONAD_DEVNET]: new Token(
    ChainId.MONAD_DEVNET,
    '0xBBC8CbBAF88417e3EC5d8ec1c8D7A1528b6596aC',
    18,
    'WMON',
    'Monad Devnet'
  ),
...
function isMonadDevnet(chainId: number): chainId is ChainId.MONAD_DEVNET {
...
class MonadTestnetNativeCurrency extends NativeCurrency {
  ...
  get wrapped(): Token {
    if (!isMonadTestnet(this.chainId)) throw new Error('Not monat testnet');
    const nativeCurrency = WRAPPED_NATIVE_CURRENCY[this.chainId];
    if (nativeCurrency) {
      return nativeCurrency;
    }
  ...
  public constructor(chainId: number) {
    if (!isMonadTestnet(chainId)) throw new Error('Not monat testnet');
...
class MonadDevnetNativeCurrency extends NativeCurrency {
  ...
  get wrapped(): Token {
    if (!isMonadDevnet(this.chainId)) throw new Error('Not monad devnet');
    const nativeCurrency = WRAPPED_NATIVE_CURRENCY[this.chainId];
    if (nativeCurrency) {
      return nativeCurrency;
    }
