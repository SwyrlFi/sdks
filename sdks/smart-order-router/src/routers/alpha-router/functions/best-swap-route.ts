...
> {
  ...
  let estimatedGasUsedUSD: CurrencyAmount;
  ...
  if (!skipUsdGasCalc && hasUsdToken) {
    ...
    log.info(
      { chainId, msg: 'MONAD_TESTNET: Skipping USD gas cost calculation' },
      'USD gas estimates of best route'
    );
  } else {
    estimatedGasUsedUSD = CurrencyAmount.fromRawAmount(usdToken, 0);
    log.info(
      { chainId, msg: 'No USD gas token configured; skipping USD gas cost calculation' },
      'USD gas estimates of best route'
    );
  }
